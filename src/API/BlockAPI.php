...
class BlockAPI{
	...
	public function init(){
		...
		$this->server->api->console->register("give", "<玩家名称> <物品[:编号]> [个数]", array($this, "commandHandler"));
	}
		...
		switch($cmd){
			case "give":
				...
				if($player instanceof Player){
					...
					if($item->getID() == 0) {
						$output .= "你不能把一个空气方块给予别人（物品名称错误）。\n";
						break;
					}
					$player->addItem($item->getID(), $item->getMetadata(), $item->count);
					$output .= "给予了 ".$item->count."个".$item->getName()." (".$item->getID().":".$item->getMetadata().") 给玩家 ".$player->username."\n";
				}else{
