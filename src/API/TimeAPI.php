...
class TimeAPI{
	...
	public function init(){
		$this->server->api->console->register("time", "<check|set|add> [时间刻]", array($this, "commandHandler"));
	}
	...
	public function commandHandler($cmd, $params, $issuer, $alias){
		...
		switch($cmd){
			case "time":
				...
				switch($p){
					case "check":
						$output .= "时间： ".$this->getDate($level).", ".$this->getPhase($level)." (".$this->get(true, $level).")\n";
						break;
					case "add":
						$output .= "将时间调整为 ".$this->add(array_shift($params), $level)."\n";
						break;
					case "set":
						$output .= "将时间调整为 ".$this->set(array_shift($params), $level)."\n";
						break;
					case "sunrise":
					case "day":
					case "sunset":
					case "night":
						$output .= "将时间调整为  ".$this->set($p, $level)."\n";
						break;
					default:
						$output .= "用法："."/time <check|set|add> [时间]\n";
						break;
				}
				break;
		}
