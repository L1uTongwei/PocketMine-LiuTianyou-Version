...
class PluginAPI extends stdClass{
	...
	public function load($file){
		if(is_link($file) or is_dir($file) or !file_exists($file)){
			console("[ERROR] ".basename($file)." 不是一个文件");
			return false;
		}
		if(strtolower(substr($file, -3)) === "pmf"){
			...
			if(preg_match_all('#([a-zA-Z0-9\-_]*)=([^\r\n]*)#u', $info, $matches) == 0){ //false or 0 matches
				console("[ERROR] 处理 ".basename($file)." 时失败");
				return false;
			}
			$info = array();
		...
		console("[INFO] 加载插件中…… \"".FORMAT_GREEN.$info["name"].FORMAT_RESET."\" ".FORMAT_AQUA.$info["version"].FORMAT_RESET." by ".FORMAT_AQUA.$info["author"].FORMAT_RESET);
		if($info["class"] !== "none" and class_exists($info["class"])){
			console("[ERROR] 加载插件时失败：类已经存在");
			return false;
		}
		if(((!isset($pmf) and (include $file) === false) or (isset($pmf) and eval($info["code"]) === false)) and $info["class"] !== "none" and !class_exists($info["class"])){
			console("[ERROR] 无法加载 {$info['name']}： 评定误差");
			return false;
		}
		...
		if(!in_array((string) CURRENT_API_VERSION, $apiversion)){
			console("[WARNING] 插件 \"".$info["name"]."\" 可能不适合当前的API版本 (".$info["apiversion"]." != ".CURRENT_API_VERSION.")! 这可能使服务器崩溃！");
		}
		...
		if($info["class"] !== "none"){			
			...
			if(!($object instanceof Plugin)){
				console("[ERROR] 插件 \"".$info["name"]."\" 不使用插件界面");
